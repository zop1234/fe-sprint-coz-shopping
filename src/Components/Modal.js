...
export default function Modal({imgUrl, title, modal, setModal, mark, setMark, data}) {
  ...
  return (
    <>
      <div className="fixed w-full h-full top-0 left-0 bg-[rgba(0, 0, 0, 0.5)] z-40" onClick={closeHandler}></div>
      <section id="modal" className={`rounded-4 ${!modal && "hidden"} duration-500 z-50`}>
        ...
        <CgClose className='absolute top-4 right-4 cursor-pointer' size="23" onClick={closeHandler} />
        <div className="absolute bottom-4 left-14 font-bold">{title}</div>
